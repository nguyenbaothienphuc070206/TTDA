...
export function buildDocuments({ techniques, videos, categories }) {
  const docs = [];

  const categoryById = new Map((categories || []).map((c) => [c.id, c]));

  for (const t of techniques || []) {
    const cat = categoryById.get(t.categoryId) || null;
    const beltId = beltFromDifficulty(t.difficulty);
    const text = formatTechniqueMarkdown({ technique: t, category: cat });
  ...
  for (const v of videos || []) {
    const text = formatVideoMarkdown({ video: v });
    ...
    docs.push({
      ...
      text,
      index: buildIndex(text),
      meta: { id: v.id, beltId: v.beltId, tags: Array.isArray(v.tags) ? v.tags : [] },
    });
  }
...
export function extractHighlights(docText, maxLines = 4) {
  ...
  const isMetaLine = (l) => {
    ...
    if (l.startsWith("Tags:")) return true;
    // Tagged metadata line: [Type: ...] [Level: ...] ...
    if (l.startsWith("[") && l.endsWith("]") && l.includes(":")) return true;
    return false;
  };

  const contentLines = lines.filter((l) => !isMetaLine(l));
  const bullets = contentLines.filter((l) => /^(-|\*)\s+/.test(l) || /^\d+\.\s+/.test(l));
  const picked = (bullets.length ? bullets : contentLines).slice(0, Math.max(1, maxLines));

  return picked.length ? picked : lines.slice(0, Math.max(1, maxLines));
}
